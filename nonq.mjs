...
class PreLoggedInUser {
    ...
    async waitInqueue(type) {
        try {
            const url = `https://tc.g4k.go.kr/ts.wseq?opcode=5101&nfid=0&prefix=NetFunnel.gRtype=5101;&sid=service_1&aid=${type}&js=yes&&${Date.now()}`;
            ...
            this.cookies['NetFunnel_ID'] = encodeURIComponent(n_cookie);
            const key = arr.split("&")[0].split("=")[1];
            while (true) {
                this.logMessage("Waiting In Queue...");
                if (resp.data.includes("nwait=0&nnext=0")) {
                    this.logMessage("Queue Passed!", "success");
                    break;
                }
                try {
                    await sleep(1000);
                    url = `https://tc.g4k.go.kr/ts.wseq?opcode=5002&key=${key}&nfid=0&prefix=NetFunnel.gRtype=5002;&ttl=1&sid=service_1&aid=${type}&js=yes&${Date.now()}`;

                    resp = await this.instance.get(url, {
                        headers: {
                            ...this.config.headers,
                            ...{
                                Cookie: buildCookieHeader(this.cookies),
    ...
    async runTask() {
        ...
        while (!res) {
            try {
                ...
                await this.waitInqueue("login_btn");
    ...
    async getTime() {
        // https://tc.g4k.go.kr/ts.wseq?opcode=5101&nfid=0&prefix=NetFunnel.gRtype=5101;&sid=service_1&aid=CIPH0801V2S1&js=yes&1737370674993
        let key = await this.waitInqueue("CIPH0801V2S1");
    ...
    async getAppointment() {
        ...
        let key = await this.waitInqueue("INSERT_VISIT");
