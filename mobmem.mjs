...
class PreLoggedInUser {
    ...
    async getAppointment() {
        // https://tc.g4k.go.kr/ts.wseq?opcode=5101&nfid=0&prefix=NetFunnel.gRtype=5101;&ttl=1&sid=service_1&aid=CIPH0801V2S1_NP&js=yes&1737346830525

        let url = `https://tc.g4k.go.kr/ts.wseq?opcode=5101&nfid=0&prefix=NetFunnel.gRtype=5101;&sid=service_1&aid=INSERT_VISIT&js=yes&${Date.now()}`;
        let resp = await this.instance.get(url, {
            headers: {
                ...this.config.headers,
                ...{
                    Cookie: buildCookieHeader(this.cookies),
        ...
        while (true) {
            ...
            try {
                await sleep(1000);
                url = `https://tc.g4k.go.kr/ts.wseq?opcode=5002&key=${key}&nfid=0&prefix=NetFunnel.gRtype=5002;&ttl=1&sid=service_1&aid=INSERT_VISIT&js=yes&${Date.now()}`;
        ...
        console.log("Last request response", last_resp);
        ...
        if (last_resp.data.result == 0) {
            console.log("Data zero: ", last_resp);
    ...
    async doLoginProcess(captcha) {
        ...
        try {
            ...
            // console.log(resp)
            if (resp.data?.errYn == 'Y') {
                this.logMessage(`Login Failed!`, "error");
                return false
            }
